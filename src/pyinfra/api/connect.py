...
async def connect_all_async(state: "State") -> None:
    ...
    task_to_host = [
        (asyncio.create_task(state.run_in_executor(host.connect)), host)
        for host in hosts
    ]
    ...
    with progress_spinner(hosts) as progress:
        for task, host in task_to_host:
            task.add_done_callback(lambda _, h=host: progress(h))
...
async def disconnect_all_async(state: "State") -> None:
    ...
    task_to_host = [
        (asyncio.create_task(state.run_in_executor(host.disconnect)), host)
        for host in hosts
    ]
